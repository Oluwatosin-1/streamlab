...
from streaming.srs_utils import get_stream_stats, start_streaming_via_srs, stop_streaming_via_srs
...
async def offer(request):
    ...
    try:
        ...
        if not stream_key:
            ...
            return JsonResponse(
                {"status": "error", "message": "Stream key is required"}, 
                status=400
            )
        ...
        try:
            ...
            return JsonResponse(
                {"status": "error", "message": "Invalid offer format"}, 
                status=400
            )
...
def connect_social(request, platform):
    ...
    try:
        ...
        return render(request, "streaming/connect.html", {
            "form": form,
            "platform": platform,
    ...
    try:
        ...
        with transaction.atomic():
            ...
            stop_streaming_via_srs(
                "live",
                session.configuration.stream_key
            )
...
class StreamingSessionDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["stream_stats"] = get_stream_stats(
            "live",
            self.object.configuration.stream_key
        )
        return context
...
def fetch_chat_messages(request):
    ...
    try:
        ...
        messages_qs = session.chat_messages.all().order_by("created_at").values(
            "user__username",
            "text",
            "created_at"
        )
...
def manage_channels(request):
    ...
    try:
        ...
        return render(request, "streaming/manage_channels.html", {
...
def validate_social_accounts(request):
    ...
    try:
        ...
        return JsonResponse({
            "status": "error",
            "message": "Failed to validate accounts"
        }, status=500)
...
def relay_status(request, session_id):
    ...
    try:
        ...
        accounts = StreamingPlatformAccount.objects.filter(
            ...
            'id', 'platform', 'rtmp_url', 'stream_key',
            'relay_status', 'relay_last_updated', 'relay_log'
        )
        ...
        return JsonResponse({
            "status": "error",
            "message": "Failed to get relay status"
        }, status=500)
...
def restart_relay(request, account_id):
    ...
    try:
        ...
        session = StreamingSession.objects.filter(
            configuration__user=request.user,
            status__in=["live", "partial"]
        ...
        return JsonResponse({
            "status": "error",
            "message": "Failed to restart relay"
        }, status=500)

@login_required
def studio_enter(request):
    """Main studio interface for streaming"""
    try:
